...
const FormModal = ({table, type, id, data, relatedData}:
 FormContainerProps & {relatedData? : any}
  ...
  const Form = () => {
    ...
    useEffect(() => {
      if(state.success){
        toast(`Subject has been beleted!`)
        setOpen(false)
        router.refresh();
      }
    }, [state])
    return type === "delete" && id ? (
      <form action={formAction} className='p-4 flex flex-col gap-4'>
        ...
        <span className='text-center font-medium'>Are you sure want to delete this {table}?</span>
  ...
  return (
    ...
    {open && (
      ...
      <div className='  bg-white rounded-md p-4 relative w-[90%] md:w-[70%] lg:w-[60%] xl:w-[50%] 2xl:w-[40%]'>
        <Form />
      <div className='absolute top-4 right-4 cursor-pointer' onClick={() => setOpen(false)}><Image src="/close.png" alt='close' width={14} height={14} /></div>
      </div>
      </div>)}
    </>
  )
