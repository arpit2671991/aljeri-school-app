...
import { classSchema, ClassSchema, subjectSchema, SubjectSchema } from '@/lib/formValidation';
...
const ClassForm = ({type, data, setOpen, relatedData }:
  {type:"create" | "update"; data?: any; setOpen:Dispatch<SetStateAction<boolean>>, relatedData?: any}) => {
    ...
    const {
      ...
      useEffect(() => {
        if(state.success){
          toast(`Class has been ${type === "create" ? "added!" : "updated."}`)
          setOpen(false)
          router.refresh();
        }
  ...
  <form className='flex flex-col gap-8 w-full' onSubmit={onSubmit}>
    <h1 className='text-xl font-semibold'>{type === "create" ? "Add Class" : "Update Class"}</h1>
